...
export default class Axis extends Component {
      ...
      <G fill='none'>
        ...
        {tickPoints.map(
           pos => <Text
                    ...
                    {typeof startVal === 'number' ? Math.round(scale(pos), 2) : this.dateToShortString(scale(pos))}
                  </Text>
         )}
      </G>
    )
  ...
  dateToShortString(scale){
      let string = '';
      if(scale instanceof Date){
        string = scale.toLocaleDateString();
      } else {
        string = new Date(scale).toLocaleDateString();
      }
      return string;
  }
