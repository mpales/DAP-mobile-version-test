...
export async function showLocation(options) {
  ...
  // currently there is only one apps which is google maps, in the future we should be able to add another from above list with changes using sendIntentandroid
  if (url && app === 'google-maps' && Platform.OS === 'android') {
    return await SendIntentAndroid.openAppWithData(
      "com.google.android.apps.maps",
      url,
    ).then(wasOpened => Promise.resolve(app));
  } else {
