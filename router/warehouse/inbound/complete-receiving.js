...
class Acknowledge extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (sku === '') {
      ...
      if (
        ...
        return {...state, _inputCode: routes[index].params.inputCode};
      }
      return {...state};
    }
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (this.props.keyStack !== nextProps.keyStack) {
      if (
        nextProps.keyStack === 'CompleteReceiving' &&
  ...
  async componentDidMount() {
    const result = await getData(
      'inboundsMobile/' + this.state._inputCode + '/photosIds',
    );
  ...
  submitItem = async () => {
    ...
    const {productID, length, width, height, volweight, weight, _inputCode} =
      this.state;
    const result = await postData(
      '/inboundsMobile/' + _inputCode + '/complete-receiving',
    );
    console.log(result);
  ...
  getPhotoReceivingGoods = async () => {
    ...
    for (let index = 0; index < receivingPhotoPostpone.length; index++) {
      ...
      await RNFetchBlob.fs
        .stat(
          Platform.OS === 'ios'
            ? receivingPhotoPostpone[index].replace('file://', '')
            : receivingPhotoPostpone[index],
        )
        .then((FSStat) => {
  ...
  uploadSubmittedPhoto = async () => {
    ...
    putBlob(
      '/inboundsMobile/' + currentASN + '/complete-receiving',
      [
      ...
      <>
              ...
              <View
                ...
                <Avatar
                  ...
                  overlayContainerStyle={{
                    backgroundColor:
                      this.props.receivingProofID !== null &&
                      this.props.receivingProofID !== this.state._inputCode
                        ? 'grey'
                        : this.props.receivingPhotoPostpone !== null
                        ? '#17B055'
                        : '#F07120',
                    flex: 2,
                    borderRadius: 5,
                ...
                <View
                  ...
                  <Text
                    ...
                    Photo Proof Complete Receiving
                  </Text>
                </View>
              </View>
            </View>
        ...
        <Overlay
          ...
          <Text style={styles.confirmText}>
            Are you sure you want Confirm Record ?
          </Text>
