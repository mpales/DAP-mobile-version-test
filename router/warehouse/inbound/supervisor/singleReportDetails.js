...
class ConnoteReportDetails extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (receivingNumber === null) {
      ...
      if (
        ...
        const photoData = Array.from({
          length: routes[index].params.arrayPhotoID.length,
        }).map((num, i) => {
          return {
            ...routes[index].params.arrayPhotoID[i],
            report_id: routes[index].params.reportID,
            inbound_id: routes[index].params.number,
            inbound_product_id: routes[index].params.productID,
          };
        });
        return {
          ...state,
  ...
  render() {
    ...
    if (dataReports === null) return <Loading />;
          ...
          <View style={[styles.body, {paddingHorizontal: 10}]}>
            <Card containerStyle={styles.cardContainer} style={styles.card}>
                ...
                <Text
                  style={[
                    styles.headerTitle,
                    {
                      ...
                      fontSize: 20,
                      color:
                        dataReports.acknowledged === 1 ? '#17B055' : '#E03B3B',
                    },
                  ]}>
                  {report_title}
                </Text>
              ...
              <View style={styles.detail}>
                <DetailList
                  title="Report By"
                  value={dataReports.reported_by.firstName}
                />
                <DetailList
                  title="Date and Time"
                  value={moment(dataReports.reported_on).format(
                    'DD/MM/YYY h:mm a',
                  )}
                />
                {dataReports.acknowledged === 1 && (
                  <DetailList
                    title="Affected Quantity"
                    value={dataReports.qty}
                  />
                )}
                ...
                {dataReports.acknowledged === 1 && (
                  <View style={{marginVertical: 20}}>
                    <Text style={styles.detailText}>Supervisor Remarks</Text>
                    <TextInput
                      style={styles.note}
                      multiline={true}
                      numberOfLines={3}
            ...
            {dataReports.acknowledged === 0 && (
              <>
                ...
                <Text style={[styles.detailText, {paddingHorizontal: 10}]}>
                  Supervisor Remarks
                </Text>
                <TextInput
                ...
                <Button
                  ...
                  disabled={
                    ...
                    this.state.resolution.length === 0
                  }
                />
              </>
            )}
