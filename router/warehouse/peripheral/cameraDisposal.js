...
class CameraSingle extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (pictureGallery === null) {
      ...
      if (
        ...
        routes[index - 1].name === 'ItemDisposalDetail'
      ) {
        if (disposalPostpone !== null)
          addMediaDisposalID(routes[index - 1].params.dataCode);
        return {
          ...state,
          pictureGallery: disposalPostpone,
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (this.props.keyStack !== nextProps.keyStack) {
      if (
        nextProps.keyStack === 'DisposalCamera' &&
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.state.rootIDType === 'ItemDisposalDetail' &&
      Array.isArray(this.props.disposalPostpone) &&
      ((prevProps.disposalPostpone === null &&
        this.props.disposalPostpone !== prevProps.disposalPostpone) ||
        this.props.disposalPostpone.length !==
          prevProps.disposalPostpone.length)
    ) {
  ...
  launchGallery = async (data) => {
    if (!data.didCancel) {
      if (data.type === undefined) {
        let stats = await RNFetchBlob.fs.stat(
          Platform.OS === 'ios' ? data.uri.replace('file://', '') : data.uri,
        );
        this.setState({
          mediaData:
            Platform.OS !== 'ios' ? 'file://' + stats.path : stats.path,
        });
      } else {
        this.setState({
          mediaData:
  ...
  render() {
    const renderItem = ({item, index}) => (
      <TouchableOpacity
        ...
        onPress={() =>
          this.props.navigation.navigate('EnlargeMedia', {
            index: index,
            ...
            <View style={styles.buttonContainer}>
              ...
              <CaptureButton
                ...
                captureContext={{
                  ...
                  setMediaType: (captureType) =>
                    this.setState({currentTypeMedia: captureType}),
                }}
              />
              <TouchableOpacity
                onPress={() =>
                  launchImageLibrary({mediaType: 'mixed'}, this.launchGallery)
                }
                style={styles.gallery}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    addMediaDisposalID: (id) =>
      dispatch({type: 'addDisposalProofID', payload: id}),
    addMediaProofPostpone: (uri) =>
      dispatch({type: 'disposalPostpone', payload: uri}),
    setBottomBar: (toggle) => {
