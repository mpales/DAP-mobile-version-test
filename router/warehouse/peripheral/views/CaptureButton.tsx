...
import type {RNCamera, TakePictureResponse, RecordResponse, TakePictureOptions} from 'react-native-camera';
...
const _CaptureButton: React.FC<Props> = ({
  ...
  const takePhotoOptions = useMemo<TakePictureOptions>(
    () => ({
      quality: 90,
    }),
    [],
  ...
  const takePhoto = useCallback(async () => {
    try {
      ...
      const photo = await camera.current.takePictureAsync(takePhotoOptions);
  ...
  const onHandlerStateChanged = useCallback(
    ...
    [isPressingButton, recordingProgress,currentTypeMedia, setIsPressingButton, startRecording, stopRecording, takePhoto],
  );
