...
class EnlargeImage extends React.Component {
    ...
    if (inboundId === null) {
      ...
      if (
        ...
        return {
          ...
          currentPictureIndex: routes[index].params.index,
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    // }
    if (
      prevState.updateLoadImage !== this.state.updateLoadImage &&
      this.state.updateLoadImage === true
    ) {
      if (this.state.convertedPictureData.length > 0) {
        for (
          let index = 0;
          index < this.state.convertedPictureData.length;
          index++
        ) {
          if (
            this.viewerImageRef[index] !== undefined &&
            this.state.convertedPictureData[index] !== undefined
          )
  ...
  handleDelete = async () => {
    ...
    let deleteString =
      this.state.typeGallery === 'receiving'
        ? '/inboundsMobile/' +
          ...
          '/complete-photo/' +
          convertedPictureData[currentPictureIndex]
        : '/inboundsMobile/' +
          this.state.inboundId +
          '/' +
          typeAPI +
    ...
    this.setState({
      ...
      convertedPictureData: convertedPictureData.filter(
        (element, index) => index !== currentPictureIndex,
      ),
      currentPictureIndex:
        currentPictureIndex > 0 ? currentPictureIndex - 1 : 0,
  ...
  handleOnChangeImage = ({viewableItems, changed}) => {
    ...
    viewableItems.forEach((element) => {
      if (
        this.viewerImageRef[element.index] !== undefined &&
        this.viewerImageRef[element.index].checkPreload() === true
      )
        this.viewerImageRef[element.index].init();
      this.setState({
        currentPictureIndex:
          element.index === currentPictureIndex
            ? currentPictureIndex
            : element.index,
      });
    });
        ...
        <View style={styles.container}>
          ...
          <View style={{backgroundColor: 'transparent', height: 20}}>
            <Text
              ...
              {'Photo Index : ' + (this.state.currentPictureIndex + 1)}
            </Text>
          </View>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    addPhotoProofUpdate: (data) =>
      dispatch({type: 'PhotoProofUpdate', payload: data}),
  };
};
