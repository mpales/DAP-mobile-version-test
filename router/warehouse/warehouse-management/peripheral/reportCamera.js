...
class CameraSingle extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (this.props.keyStack !== nextProps.keyStack) {
      if (
        nextProps.keyStack === 'StockTakeReportCamera' &&
        this.props.keyStack === 'EnlargeImage'
      ) {
        this.setState({pictureGallery: this.props.stockTakeReportPhotoList});
        return false;
      }
    }
    return true;
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.stockTakeReportPhotoList.length !==
      this.props.stockTakeReportPhotoList.length
    ) {
      this.setState({
        pictureGallery: this.props.stockTakeReportPhotoList,
      });
  ...
  handlePhotoConfirmation = async (confirm) => {
    if (confirm) {
      let newStockTakeReportPhotoList = [
        ...this.props.stockTakeReportPhotoList,
      ];
      newStockTakeReportPhotoList.push(this.state.pictureData);
      await this.props.addStockTakeReportPhotoList(newStockTakeReportPhotoList);
    }
  ...
  render() {
    ...
    console.log('typeof', this.state.pictureData);
