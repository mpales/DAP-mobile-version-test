...
class EnlargeImage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      pictureData: this.props.recollectionPhotoList,
  ...
  handleDelete = () => {
    let newPictureData = [...this.props.recollectionPhotoList];
    ...
    this.props.addRecollectionPhotoList(newPictureData);
    if (newPictureData.length < 1) {
      this.props.setRecollectionPhotoSubmitted(false);
