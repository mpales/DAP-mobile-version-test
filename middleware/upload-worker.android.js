...
export const applyWorker = () => {
    ...
    return (createStore) => (reducer, initialState, enhancer) => {
      // // New reducer for workified store
       let replacementReducerOffline = (state, action) => {
        if (action.state) {
          if(persist === true){
            subscriptions = action.state.originReducer.persistUploadSubscriptions;
          } else if(persist === false){
            action.state.originReducer.persistUploadSubscriptions = subscriptions;
          } 
          return action.state;
        }
        return state;
      ...
      let store =  createStore(replacementReducerOffline, reducer({}, {}), enhancer);
      ...
      // // Replace dispatcher
       store.dispatch = async (action) => {
        if(action.type === 'logout' || action.type === 'DeviceSignature'){
          persist = false;
        } else if(action.type === 'persist/REHYDRATE' && persist === false && hydrated === false){
          ...
          hydrated = true;
          persist = null;
        } else if(action.type !== undefined && (action.type !== 'persist/PERSIST' && action.type !== 'persist/REHYDRATE' && action.type !== '@@network-connectivity/CONNECTION_CHANGE' && action.type !== 'resetLogin' && action.type !== 'loggedIn') && persist === false && hydrated === true){
  ...
  const initFetchFromHydrated =  async () => {
    ...
    for (let index = 0; index < currentSubscriptions.length; index++) {
        if(currentSubscriptions[index] !== undefined && currentSubscriptions[index] !== null){
          currentSubscriptions[index].persist = currentPersist === null ? true : currentPersist ;
        }
    }
  ...
  const applyBackgroundFetch =  async (options = {}) => {
    ...
    return assign(worker, Object.create({clearSubs, applyBackgroundFetch, applyWatchUploadEnhancher}));
...
class NetworkWorker {
  ...
  runTask = async () =>{
    let apiFetch = fetchDefaults(crossFetch, SERVER_DOMAIN, async (url, opt) => {
      return {
        headers: {
    ...
    let blobFetch = fetchBlobDefault(RNFetchBlob, SERVER_DOMAIN, async (url, opt, data) => {
    ...
    if(currentTask[index] !== null){  
      ...
      if(element !== null && !element === false && element.headers !== undefined){
        result = await apiFetch(url, {
         ...element,
        });
        if(result.status === 200 || result.status === 401 || result.status === 400|| result.status === 403){
          this.pendingTask[index] = null;
          ReactNativeForegroundService.update({
            id: 155+index,
            title: 'Upload being sync to server',
            message: 'Your action has been saved to server by background process',
            importance: 'default',
            mainOnPress: async () => {
